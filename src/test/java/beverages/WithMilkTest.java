...
public class WithMilkTest {
    ...
    public void add_ten_cents_to_original_beverage_price() {
        ...
        assertThat(REGULAR_COFFEE_PRICE + MILK_OVERHEAD, is(closeTo(coffeeWithMilk.price(), PRECISION)));

        Tea tea = new Tea();
        WithMilk teaWithMilk = new WithMilk(tea);
        assertThat(REGULAR_TEA_PRICE + MILK_OVERHEAD, is(closeTo(teaWithMilk.price(), PRECISION)));

        HotChocolate hotChocolate = new HotChocolate();
        WithMilk hotChocolateWithMilk = new WithMilk(hotChocolate);
        assertThat(REGULAR_HOT_CHOCOLATE_PRICE + MILK_OVERHEAD, is(closeTo(hotChocolateWithMilk.price(), PRECISION)));
    }
}